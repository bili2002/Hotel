...
Date Date::getDate() {
    std::cout<<"Въведете дата (във формата м д г)";

    Date temp;

    std::cin>>temp.month>>temp.day>>temp.year;

    if (temp.month < January || temp.month > December) {
        std::cout<<"Грешна дата, въведете наново: \n";
        getDate();
    } 
    ...
    if (temp < getCurrentDate()) {
        std::cout<<"Датата е преди днешната! Въведете наново: \n";
        getDate();
    }
